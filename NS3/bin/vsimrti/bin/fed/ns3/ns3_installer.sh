...
patch_ns3()
{
   ...
   # this is done by premake5 now
   #### generate protobuf sources
   #if [ "$arg_regen_protobuf" = "true" ]; then
   #   protoc --cpp_out=./federate --proto_path=federate federate/ClientServerChannelMessages.proto
   #fi
}

unify_folder_name()
{
    # do we really need a third time scrambling everything?
...
build_ns3()
{
  ...
  #CXXFLAGS=-std=c++11
  #./waf configure --disable-python --disable-nsclick --disable-tests --disable-examples --disable-gtk --disable-nsc --enable-modules=VSimRTI
  #./waf build $arg_make_parallel
  python ./build.py --disable-netanim

  log "Build ns3-federate"
  cd ${current_dir}/federate
  mv src/VSimRTI/model/ClientServerChannel.h .
  mv src/VSimRTI/model/ClientServerChannel.cc .
  if [ -f src/VSimRTI/model/ClientServerChannelMessages.pb.h ]; then
    rm src/VSimRTI/model/ClientServerChannelMessages.pb.h
  fi
  if [ -f src/VSimRTI/model/ClientServerChannelMessages.pb.cc ]; then
    rm src/VSimRTI/model/ClientServerChannelMessages.pb.cc
  fi
...
deploy_ns3()
{
    if [ "$arg_deploy" == "true" ]; then
        ...
        cp federate/bin/ns3-federate "$ns3_deploy_folder/build/scratch/VSimRTI_Starter"
