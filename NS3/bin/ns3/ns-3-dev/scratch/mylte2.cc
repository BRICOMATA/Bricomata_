...
{
//for (double distance = 1000;distance<=3000;distance=distance+1000){
	//std::cout<<"At Distance = "<<distance<<std::endl;
	std::cout<<"=============="<<std::endl;
	for (uint16_t numberOfUENodes =5;numberOfUENodes<=75;numberOfUENodes=numberOfUENodes+10){
		 ...
		 // std::cout<<"Ip Address of Remote Host: "<<internetIpIfaces.GetAddress(1)<<std::endl;
		  ...
		  for(uint32_t u=0;u<ueNodes.GetN();u++){
			 ...
			 // cvmPed->SetVelocity(Vector (1.4, 0, 0)); //move Pedestrian to right 1.4.0m/s
			  cvmVeh= ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>();
			  cvmVeh->SetVelocity(Vector (22.22, 0, 0)); //move Vehicle to right 60.0m/s
			  u=u+1;
		  }
		  //Ptr<ConstantVelocityMobilityModel> cvmPed = ueNodes.Get(1)->GetObject<ConstantVelocityMobilityModel>();
		  //cvmPed->SetVelocity(Vector (1.4, 0, 0)); //move Pedestrian to right 1.4.0m/s

		  //================================== Fading Traces ============================
		  lteHelper->SetFadingModel("ns3::TraceFadingLossModel");
		  lteHelper->SetFadingModelAttribute ("TraceFilename", StringValue ("src/lte/model/fading-traces/fading_trace_EVA_80kmph.fad"));
		 ...
		 // std::cout<<"Ip Address of UE Node 2: "<<ueIpIface.GetAddress(1)<<std::endl;
		  ...
		  clientApps.Start (Seconds (1.5)); // all clients start at the same time - is the reason for high PLR
		...
		//============================== Output Result ============================================
			...
			Ptr<Ipv4FlowClassifier> classifier = DynamicCast<Ipv4FlowClassifier> (flowHelper.GetClassifier ());
			 ...
			 // pedDelay = delay2+delay4;
			  std::cout<<"#Nodes = "<<numberOfUENodes<<std::endl;
			  //std::cout <<"\tDelay = "<<delay/numberOfUENodes<<std::endl;
			  std::cout <<"\tPLR= "<<rlp/(2*numberOfUENodes)<<std::endl;
		//========================================================================================
		  Simulator::Destroy();
		}
    //}
