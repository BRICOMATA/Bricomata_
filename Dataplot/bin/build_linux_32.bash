#!/bin/sh
#
#  Name:     build_linux_32.bash
#  Purpose:  This is a build script for Dataplot.
#
#            1. This uses the bash shell.  There is also a
#               version using the c shell.
#
#               This script has been used for various Red Hat
#               (Cent OS, Fedora, cygwin) systems and Ubuntu (16.04).
#               It should be straightforward to use it for other Linux
#               variants.  The main issue for different variants
#               of Linux/Unix is where various libraries are
#               located.
#
...
#               i. HAVE_ISNAN = Specify whether the ISNAN function is
#                  available (off for versions older than 4.1.2, on for
#                  versions newer than 4.1.2).
#               j. HAVE_EXECUTE_COMMAND_LINE = Specify whether the
#                  EXECUTE_COMMAND_LINE function is available.  Support
#                  was added in version 4.6 of gfortran.
#               k. Dataplot can optionally take advantage of several
...
#                   ix. HAVE_UBUNTU - for Ubuntu
#                       HAVE_REDHAT - for RedHat Linux (Fedora,
#                                     CentOS, cygwin)
#
set -x
rm -f *.o
...
#
#SRCDIR=/usr/local/src/dataplot
#SRCDIR=/home/heckert/dataplot/src
#SRCDIR=/home/heckert/src/dataplot
#SRCDIR=/home/heckert/src/dataplot
SRCDIR=./
DPNAME=./dataplot
FCOMP="gfortran"
...
HAVE_ISNAN=on
#HAVE_ISNAN=off
HAVE_EXECUTE_COMMAND_LINE=on
#HAVE_EXECUTE_COMMAND_LINE=off
#
...
#HAVE_GKS=on
#
#  Specify Linux variant.  This mostly used to specify the
#  default locations for libraries and include files.  I
#  have CentOS, Fedora, Cygwin, and Ubuntu 16.04 systems,
#  other Linux variants may need to use different default
#  locations.  Set one of the following to "on".
#
HAVE_RED_HAT=on
#HAVE_RED_HAT=off
#HAVE_UBUNTU=on
...
#
if [ $HAVE_RED_HAT = "on" ]; then
   LDFLAG="-L$HOME/lib -L/usr/lib64 -L/usr/lib  -L/usr/local/lib"
   CFLAGS="-ansi -c -m64"
fi
#
#  For Ubuntu systems, developer packages need to be installed
#  in order to locate the ".h" files.  Specifically,
#
#    sudo apt install libx11-dev       - for X11
#
#    sudo apt install libgd-dev        - for libgd
#    sudo apt install libfreetype6     - for libgd
#    sudo apt install libfreetype6-dev - for libgd
#
#    sudo apt install libplot-dev      - for libplot
#    sudo apt install libplot2c2       - for libplot
#
#    sudo apt install libreadline6     - for libreadline
#    sudo apt install libreadline6-dev - for libreadline
#
if [ $HAVE_UBUNTU = "on" ]; then
   LDFLAG="-L/usr/lib/x86_64-linux-gnu -L/usr/lib  -L/usr/local/lib"
   CFLAGS="-ansi -c -m64"
fi
#
FFLAGS2="$IOPT -DLINUX -fdefault-real-8 -fdefault-double-8"
CFLAGS2=" "
#
if [ $LARGE_MODEL = "on" ]; then
   FFLAGS2="$FFLAGS2 -mcmodel=medium"
   CFLAGS="$CFLAGS -mcmodel=medium"
fi
#
if [ $HAVE_ISNAN = "on" ]; then
   FFLAGS2="$FFLAGS2 -DHAVE_ISNAN
fi
#
if [ $HAVE_EXECUTE_COMMAND_LINE = "on" ]; then
   FFLAGS2="$FFLAGS2 -DHAVE_EXECUTE_COMMAND_LINE
fi
#
FFLAGS3="-DDD -DI32"
if [ $IPREC = "64-64" ]; then
   FFLAGS2="$IOPT -fdefault-real-8  -fdefault-double-8"
   FFLAGS3="-DDD -DI32"
   CFLAGS2="-DDOUBLE"
elif [ $IPREC = "32-64" ]; then
   FFLAGS2="$IOPT -fdefault-double-8"
   FFLAGS3="-DSD  -DI32"
   CFLAGS2=" "
fi
CFLAGS="$CFLAGS -I./ -I$HOME/include -I/usr/include -I/usr/local/include"
FFLAGS="-c $FFLAGS2"
FFLAGS4=" "
CFLAG3=" "
#
if [ $HAVE_GD = "on" ]; then
   LDFLAG="$LDFLAG -lgd -lpng -ljpeg -lz -lfontconfig -lfreetype -lpthread"
   FFLAGS4="$FFLAGS4  -DHAVE_GD"
   if [ $HAVE_TIFF = "on" ]; then
      CFLAGS3="$CFLAGS3 -DHAVE_LIBTIFF"
      LDFLAG="$LDFLAG -ltiff"
   fi
   if [ $HAVE_VPX = "on" ]; then
      CFLAGS3="$CFLAGS3 -DHAVE_LIBVPX"
      LDFLAG="$LDFLAG -lvpx"
   fi
fi
#
if [ $HAVE_CAIRO = "on" ]; then
   LDFLAG="$LDFLAG -lcairo"
   FFLAGS4="$FFLAGS4  -DHAVE_CAIRO"
   if [ $HAVE_X11 = "on" ]; then
      CFLAGS="$CFLAGS -DHAVE_X11";
   fi
   CFLAGS="$CFLAGS -I/usr/include/cairo"
fi
#
if [ $HAVE_OPEN_GL = "on" ]; then
  LDFLAG="$LDFLAG -lGLU -lGL"
  FFLAGS4="$FFLAGS4  -DHAVE_OPEN_GL"
fi
#
if [ $HAVE_GKS = "on" ]; then
  LDFLAG="$LDFLAG -lgks"
  FFLAGS4="$FFLAGS4  -DHAVE_OPEN_GKS"
fi
#
...
#
if [ $HAVE_LIBPLOT = "on" ]; then
  LDFLAG="$LDFLAG -lplot -lXaw -lXmu -lXt -lSM -lICE -lXext"
  FFLAGS4="$FFLAGS4  -DHAVE_LIBPLOT"
fi
#
...
#
if [ $HAVE_X11 = "on" ]; then
      LDFLAG="$LDFLAG -L/usr/X11R6/lib -lX11"
      FFLAGS4="$FFLAGS4  -DHAVE_X11"
fi
#
...
#
if [ $HAVE_RL = "on" ]; then
  LDFLAG="$LDFLAG -lreadline -lhistory"
  locate libncurses
  if [ "$?" = "0" ]; then
     LDFLAG="$LDFLAG -lreadline -lhistory -lncurses"
     FFLAGS4="$FFLAGS4  -DHAVE_READLINE"
  else
     locate libtermcap
     if [ "$?" = "0" ]; then
