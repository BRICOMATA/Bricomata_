...
#+
		...
		action.connect('activate', self.worker.fold_current_block)
...
#+
		self.window.lookup_action("fold").set_enabled(self.window.get_active_document() != None)
################################################################
...
class FoldingAppActivatable(GObject.Object, Gedit.AppActivatable):
    ...
    def do_activate(self):
        self.app.add_accelerator("<<Alt><Shift>C", "win.fold", None)
