from gi.repository import GObject, Gtk, Gedit, Gdk, GtkSource, Gio
...
lang_support = {
	...
	'LaTex':{'startPattern':'\\section','stopPattern':'\\section'},
...
#		self.insert_menu()
#+
		action = Gio.SimpleAction(name="folda")
		action.connect('activate', self.worker.on_toggle_all)
...
#		self.remove_menu()
#+
		self.window.remove_action("folda")
...
#+
		self.window.lookup_action("folda").set_enabled(self.window.get_active_document() != None)
################################################################
...
class CodeFolder(GObject.Object):
	def __init__(self,window):
		self.window = window
	def on_toggle_all(self,j,action):
...
class FoldingAppActivatable(GObject.Object, Gedit.AppActivatable):
    ...
    def do_activate(self):
        self.app.add_accelerator("<<Alt><Shift>C", "win.folda", None)

        self.menu_ext = self.extend_menu("tools-section")
        item = Gio.MenuItem.new(_("folda"), "win.folda")
        self.menu_ext.append_menu_item(item)

    def do_deactivate(self):
        self.app.remove_accelerator("win.folda", None)
        self.menu_ext = None
