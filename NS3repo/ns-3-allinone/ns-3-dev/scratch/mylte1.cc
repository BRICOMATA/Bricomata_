 ...
 * attaches several UE to the eNodeB starts a flow for each UE to  and from a remote host.
 */
...
main (int argc, char *argv[]) {
  ...
  for (uint16_t numberOfUENodes = 5;numberOfUENodes<=100;numberOfUENodes=numberOfUENodes+10){
  uint16_t maxNumberOfPackets = 101;
  ...
  //std::cout<<" Packet Dispatch Interval = "<<interPacketInterval<<std::endl;

  //for(int d=500;d<=distance;d=d+1000){
	 ...
	 // std::cout<<"Ip Address of Remote Host: "<<internetIpIfaces.GetAddress(1)<<std::endl;
	  ...
	  for (uint32_t u = 0; u < ueNodes.GetN (); ++u){
		  positionAllocUE->Add(Vector(u*5+distance, 0, 0));
		  //positionAllocUE->Add(Vector(d, 0, 0));
	  }
	  mobility.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
	  mobility.SetPositionAllocator(positionAllocUE);
	  mobility.Install(ueNodes);

	 // uint32_t p = 0.7*ueNodes.GetN ();
	 // Ptr<ConstantVelocityMobilityModel> cvmVeh;
	  ...
	  for (uint32_t u = 0; u < ueNodes.GetN (); ++u){
		/* if(u<=p){
			 ...
			 cvmVeh->SetVelocity(Vector (20, 0, 0)); //move Vehicle to right with the speed 20.0m/s
		  }else{*/
			 cvmPed = ueNodes.Get(u)->GetObject<ConstantVelocityMobilityModel>();
	 ...
	 // std::cout<<"Ip Address of UE Node 2: "<<ueIpIface.GetAddress(1)<<std::endl;
	  ...
	  for (uint32_t u = 0; u < ueNodes.GetN (); ++u) {
		  ...
		  dlClient.SetAttribute ("MaxPackets", UintegerValue(maxNumberOfPackets));

		  UdpClientHelper ulClient (remoteHostAddr, ulPort);
		  ulClient.SetAttribute ("Interval", TimeValue (Seconds(interPacketInterval)));
		  ulClient.SetAttribute ("PacketSize", UintegerValue (packetSize));
		  ulClient.SetAttribute ("MaxPackets", UintegerValue(maxNumberOfPackets));

		  UdpClientHelper client (ueIpIface.GetAddress (u), otherPort);
		  client.SetAttribute ("Interval", TimeValue (Seconds(interPacketInterval)));
		  client.SetAttribute ("PacketSize", UintegerValue (packetSize));
		  client.SetAttribute ("MaxPackets", UintegerValue(maxNumberOfPackets));
		...
		Ptr<Ipv4FlowClassifier> classifier = DynamicCast<Ipv4FlowClassifier> (flowHelper.GetClassifier ());
			...
			for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter){
				/*  Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow (iter->first);
				  ...
				  float lr = float(100*((iter->second.txPackets - iter->second.rxPackets))/float(iter->second.txPackets));
					  if(iter->first==1)
						  lr1 = lr;
				...
				if(iter->second.rxPackets!=0){
					plr = plr+float((100*(iter->second.txPackets - iter->second.rxPackets))/float(iter->second.txPackets));
		  ...
		  plr=plr/numberOfUENodes;
		  delay=delay/numberOfUENodes;
		  std::cout<<"Number of UEs (Pedestrians) = "<<numberOfUENodes<<std::endl;
		  std::cout <<"\tAverage Delay = "<<delay/numberOfUENodes<<"ms"<<std::endl;
		  std::cout <<"\tPacket Loss Ratio =  "<<plr/numberOfUENodes<<"%"<<std::endl;
	//========================================================================================
